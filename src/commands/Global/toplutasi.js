...
module.exports = {
    name: "toplutaşı",
    aliases: ["toplu-taşı", "toplu-tasi", "toplutasi"],
    execute: async (client, message, args, embed, author, channel, guild) => {
  if (!message.member.roles.cache.has(config.Guild.emperor) && !message.member.hasPermission("ADMINISTRATOR")) return channel.send(embed.setDescription("Komutu kullanabilmek için geçerli yetkin olmalı.")).then(x => x.delete({timeout: 5000}));
  if (!message.member.roles.cache.has(config.Guild.emperor) && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(embed.setDescription(`Toplu taşı komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`)).then(x => x.delete({timeout: 5000}));
