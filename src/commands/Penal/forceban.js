...
module.exports = {
  name: "forceban",
  aliases: ["beta", "sydlaxy", "morgraine", "mustafa", "mk", "anka", "cu", "tabiefendim", "ANKA", "meka", "force-ban", "force", "yargıç", "yargıc", "yargic", "YARGIÇ"],
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    if (!user) return channel.send(embed.setDescription('Kalıcı olarak banlanacak kullanıcıyı belirtmelisin.'))
    ...
    await user.send(`<a:ashera_banned:993717896649904238> ${author} adlı yetkili tarafından **${reason}** sebebiyle **ASHERA** sunucusundan kalıcı olarak banlandın.`).catch()
    await user.ban(user, { reason: reason });
    message.channel.send((`**${user}** kullanıcısı **${reason}** sebebiyle sunucudan kalıcı olarak banlandı! (Ceza Numarası: \`#${db.fetch(`ceza_${guild.id}`)}\`)`))
    ...
    const log = new Discord.MessageEmbed()
      ...
      .setDescription(`
      ${user ? user.toString() : user.username} kişisi sunucudan kalıcı olarak banlandı!
    ...
    db.push(`sicil_${user.id}`, `${author} tarafından ${moment(Date.now()).format("LLL")} tarihinde **${reason}** sebebiyle kalıcı olarak **BAN** cezası almış.`)
