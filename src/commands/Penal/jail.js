...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (!message.member.roles.cache.has(config.penals.jail.staff) && !message.member.hasPermission(config.Guild.emperor)) return channel.send("Komutu kullanabilmek için geçerli yetkin olmalı.");
        ...
        member.voice.kick()
        message.channel.send((`**${member}** kullanıcısı **${sebep}** sebebiyle karantinaya atıldı! (Ceza Numarası: \`#${db.fetch(`ceza_${guild.id}`)}\`)`))
