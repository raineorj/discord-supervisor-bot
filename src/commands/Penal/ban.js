...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    if (!message.member.roles.cache.has(config.penals.ban.staff) && !message.member.hasPermission(config.Guild.emperor)) return channel.send(embed.setDescription("Komutu kullanabilmek için geçerli yetkin olmalı."))
    ...
    message.channel.send((`<a:ashera_banned:993717896649904238> **${user}** kullanıcısı **${reason}** sebebiyle sunucudan banlandı! (Ceza Numarası: \`#${db.fetch(`ceza_${guild.id}`)}\`)`))
    ...
    db.push(`sicil_${user.id}`, `${author} tarafından __**${moment(Date.now()).format("LLL")}**__ tarihinde **${reason}** sebebiyle **BAN** cezası almış.`)
